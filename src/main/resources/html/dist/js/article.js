...
$(function () {
    ...
    $("#jqGrid").jqGrid({
        ...
        colModel: [
            ...
            {label: '文章标题', name: 'title', index: 'title', width: 240},
            {label: '添加人', name: 'nickname', index: 'nickname', width: 120},
...
$('#saveButton').click(function () {
    ...
    if (validObject()) {
        ...
        var data = {"nickname": addName, "title": title, "contentHtml": contentHtml};
        var url = '/admin/article/save';
        //id>0表示编辑操作
        if (id > 0) {
            data = {"id": id, "title": title, "contentHtml": contentHtml, "nickname": addName};
...
function articleEdit() {
    ...
    //请求数据
    $.get("/admin/article/info/" + id, function (r) {
        if (r.code == 0 && r.data != null) {
            //填充数据至modal
            $('#articleId').val(r.data.id);
            $('#articleName').val(r.data.title);
            $('#articleAuthor').val(r.data.nickname);
            editor.html('');
            editor.html(r.data.contentHtml);
        }
    });
    //显示modal
    $('#articleModal').modal('show');
